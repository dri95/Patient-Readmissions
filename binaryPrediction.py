...
def preprocess(dataset):
   ...
   #dataset = dataset.drop(['encounter_id2','patient_nbr2'], axis = 1)
...
print("Accuracy is {0:.2f}".format(accuracy_score(y_val, pred)))
print("Precision is {0:.2f}".format(precision_score(y_val, pred)))
print("Recall is {0:.2f}".format(recall_score(y_val, pred)))
print("AUC is {0:.2f}".format(roc_auc_score(y_val, pred)))
...
#testrem.groupby('readmitted').size()
predictest.to_csv('binary.csv', header=True, index=True)
